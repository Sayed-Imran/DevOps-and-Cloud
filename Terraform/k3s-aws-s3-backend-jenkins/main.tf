...
resource "aws_instance" "k3s-master-aws" {
  ami           = var.ami
...
resource "aws_instance" "k3s-worker1-aws" {
  ...
  provisioner "remote-exec" {
    ...
    inline = [
      "curl -sfL https://get.k3s.io | INSTALL_K3S_VERSION=\"v1.25.5+k3s1\" sh -s - agent --server https://${aws_instance.k3s-master-aws.private_ip}:6443 --token=${var.token}"
